...
import { ImageHeaderThreeColumnSection } from '@/app/(frontend)/_interfaces/pages'
...
export default function ImageHeaderThreeColumn({ data, domainBlob }: Props) {
  const hasImage = data.sectionHeaderImage?.filename
  ...
  return (
    <div className="">
      <div className="relative w-full h-[100px] sm:h-[120px] md:h-[160px] lg:h-[250px] xl:h-[300px] 2xl:h-[400px]">
        {hasImage && (
          <Image
            src={domainBlob + data.sectionHeaderImage.filename}
            alt={data.sectionHeaderImage.filename}
            fill
