...
export default function Product({products, domainBlob}:{products:ProductItem[], domainBlob:string}) {
    return (
        <div className="w-full relative">
            <Image priority className="h-auto w-[160px] absolute bottom-0 left-0 z-0 sm:w-[340px] md:w-[430px]" src={background} alt={""} width={0} height={0}/>
                ...
                <div className="flex flex-col gap-y-6 items-center justify-center">
                    ...
                    <div className="grid grid-cols-2 gap-4 justify-items-center lg:gap-7">
                        {products.map((src, idx) => (
                            <div key={idx} className="rounded-[10px] shadow-md md:shadow-lg md:rounded-[20px] bg-center bg-cover overflow-hidden w-[130px] h-[90px] sm:w-[260px] sm:h-[160px] md:w-[280px] md:h-[200px] lg:w-[440px] lg:h-[300px]" style={{ backgroundImage: `url('${domainBlob+src.productImage.filename}')`}}></div>
                        ))}
                    </div>
                </div>
            </div>
